...
def index():
    ...
    ordens = db.execute(
        "SELECT symbol, name, sum(shares) as sumshares, price FROM ordens WHERE user_id = ? GROUP BY symbol",
        session["user_id"],
    )
    account = cash[0]["cash"]
    for order in ordens:
    ...
    return render_template(
        "index.html", ordens=ordens, cash=cash[0]["cash"], account=account
    )
...
def buy():
    ...
    if request.method == "GET":
        ...
        db.execute(
            "INSERT INTO ordens (user_id, symbol, shares, name, price, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
            session["user_id"],
            symbol.upper(),
            shares,
            quote["name"],
...
def history():
    ...
    ordens = db.execute(
        "SELECT symbol, name, shares, price, timestamp FROM ordens WHERE user_id = ?",
        session["user_id"],
    )
    if not ordens:
        ...
        for order in ordens:
            if order["shares"] > 0:
                ...
                order["status"] = "Sold"
        return render_template("history.html", ordens=ordens)
...
def sell():
    ...
    if request.method == "GET":
        symbols = db.execute(
            "SELECT symbol FROM ordens WHERE user_id = ? GROUP BY symbol",
            session["user_id"],
        )
        ...
        sumshares = db.execute(
            "SELECT symbol, price, name, SUM(shares) as sumshares FROM ordens WHERE user_id = ? AND symbol = ?",
            session["user_id"],
            symbol,
        )
